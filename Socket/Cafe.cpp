...
/*! \brief Compress string \c inMessage using compression level \c inCompressionLevel, and return result through string \c outMessage.
...
void Socket::Cafe::compress(const std::string& inMessage, std::string& outMessage, unsigned int inCompressionLevel)
{
   ...
   else
   {
      ...
      unsigned int lBufLength = (inMessage.size() + inMessage.size()/1000 + 1) + 12;
      outMessage.resize(lBufLength);
      // compress message
      int lReturn = ::compress2((Bytef *)outMessage.data(), (uLongf*)&lBufLength, (const Bytef *)inMessage.data(), inMessage.size(), inCompressionLevel);
      // check for error
      if(lReturn != Z_OK) 
      {
         outMessage.clear();
...
/*! \brief Receive \c inCount bytes from socket.
   ...
   receive((char*)&lSignature, sizeof(lSignature));
	switch(ntohl(lSignature)) 
	{
		case 0xCAFE: // uncompressed Cafe
		{
			...
			// read message body
			receive((char*)outMessage.data(), lMessageSize);
			break;
		}
...
/*! \brief Send string message \c inMessage to connected server using the 0xcafe protocol.

This function sends a message string using either the Cafe protocol. The user can specify a compression level using argument \c inCompressionLevel. However, this argument is ignored by default and messages are not compressed in any way. To enable compression, the class must be compiled with variable PACC_ZLIB set, and the executable must be linked with the zlib library <http://www.gzip.org/zlib>. 

Compression level must range from 0 to 9. A nul compression level means no compression, while a level of 9 specifies maximum compression. The message is sent compressed only if smaller than the original.

Any error raises a Socket::Exception. For instance, it throws an exception with code Socket::eConnectionClosed if the connection is closed by the other party during message transmission, or with code Socket::eTimeOut if the message cannot be sent before the time out period expires. The time out period can be changed using function Port::setSockOpt with parameter Socket::eSendTimeOut.
 */
...
/*! \brief Uncompress string \c ioMessage knowing that the uncompressed message length is \c inUncompressedSize, and return result through string \c ioMessage.

WARNING: in order to enable message compression/uncompression, this class needs to be compiled with variable PACC_ZLIB set.
*/
#ifdef PACC_ZLIB
void Socket::Cafe::uncompress(std::string& ioMessage, unsigned int inUncompressedSize)
{
   string lUncompressedMessage;
   lUncompressedMessage.resize(inUncompressedSize);
   int lReturn = ::uncompress((Bytef*)lUncompressedMessage.data(), (uLong*)&inUncompressedSize, (const Bytef*)ioMessage.data(), ioMessage.size());
   if(lReturn != Z_OK) 
   {
